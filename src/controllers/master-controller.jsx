...
export default function MasterController () {
    ...
    const dispatch = WebMethods.GetReduxDispatch()
     ...
     // get data
    const fetchData = async (pageno, sortdata, pagelimit) => {
        const payload = {
            paginationinfo : {
        ...
        if(response.status === 200){
            ...
            if(reduxStore.pageRights.apiName === 'pointreport'){
                dispatch(setData({loginData : response.otherdata}))
            }
        }
    }
    ...
    const venderDetail = async () => {
        const vendorRes = await WebMethods.GetListRequest(`${serverURL}point`, {})
        if(vendorRes.status === 200){
            dispatch(setData({loginData : vendorRes.data}))
        }
    }
    ...
    useEffect(()=>{
        dispatch(clearData())
        fetchData()
        if(reduxStore.pageRights.apiName === 'customer'){
            venderDetail()
        }
    },[])
    ...
    const handleSubmitForm = async (form) => {
        dispatch(setData({disabledbutton : { submit : true} }))
        ...
        if(response.status === 200){
          ...
          handleCloseForm()
          fetchData()
          dispatch(setData({disabledbutton : { submit : false} }))
        }else{
    ...
    const handleDownloadExcel = async () => {
        dispatch(setData({disabledbutton : { export : true} }))
        const response = await WebMethods.GetListRequest(`${serverURL}download-csv`,{ api : reduxStore.pageRights.apiName})
        ...
        link.download = 'your_data.csv';

        // Append the link to the document and trigger the download
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        dispatch(setData({disabledbutton : { export : false} }))
    }
